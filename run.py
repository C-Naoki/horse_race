...
def predict(data_c, date=sets.date.replace('/', ''), venue_id_list=sets.venue_id_list):
    # dateからdayを取得
    if date[6] == '0':
        day = date[7]
    else:
    ...
    data_c = data_c[data_c['date'] < datetime.date(int(sets.year), int(sets.month), int(day))]
    # 説明変数と目的変数に分割
    X = data_c.drop(sets.drop_list+['rank', 'date', 'popular'], axis=1)
    y = data_c['rank']
    ...
    better_fukusho_df = pd.DataFrame([], columns=['race_id', 'horse_number', 'predict_order', 'odds', 'score'])
    # 出馬表データのスクレイピング
    ...
    for venue_id in venue_id_list:
        race_id_ls = [venue_id+str(i).zfill(2) for i in range(1, 13)]
        ...
        original_race_card = OriginalRaceCard.scrape(race_id_ls)
        _df = table_merger(original_race_card.preprocessed_df, 'original')
        # ModelEvaluatorインスタンスの作成
        model_evaluator = ModelEvaluator(lgb_clf)
        X_fact = _df.drop(sets.drop_list+['date'], axis=1)
        # 各レースの本命馬、対抗馬、単穴馬、連下馬の出力
        pred = model_evaluator.predict_proba(X_fact, train=False)
        proba_table = _df[['horse_number']].astype('int').copy()
        ...
        try:
            ...
            today_return_tables, arrival_tables_df = Refund.scrape(race_id_ls)
            rt = Refund(today_return_tables)
        ...
        # 三連複が的中したかどうか記録する配列
        hits_sanrenpuku = [0] * original_race_card.af_df.index.unique().shape[0]
        for i, proba in enumerate(proba_table.groupby(level=0)):
            # 使う頻度の高い数値を変数に格納
            race_proba = proba[1].sort_values('score', ascending=False).head(6)
            race_class = m.make_race_class(original_race_card, proba)
            race_id = proba[0]
            if race_id[-2] == '0':
                race_num = ''+race_id[-1]
            else:
            ...
            if len(today_odds_df):
                ...
                today_df['score'] = proba[1].sort_values('horse_number').set_index('horse_number')
                # あるレースの買うべき複勝馬券を計算する
                for horse_number, row in today_df.iterrows():
                    if m.get_better_fukusho(fukusho_odds_df, row['score'], row['fukusho_odds']):
                        better_fukusho_df = pd.concat([
                                ...
                                pd.DataFrame([[
                                    race_id,
                                    horse_number,
                                    list(proba[1].sort_values('score', ascending=False)['horse_number']).index(int(horse_number))+1,
                                    row['fukusho_odds'],
                                    row['score']
                ...
                # 買うべき複勝馬に星マークをつける
                for _num in better_fukusho_df[better_fukusho_df['race_id'] == race_id]['horse_number']:
                    for i, _proba in enumerate(_race_proba.iterrows()):
                        if '☆' not in str(_proba[1]['horse_number']) and int(_proba[1]['horse_number']) == _num:
            ...
            if exists_return_tables:
                ...
                real_arrival = list()
                for j in range(6):
                    if len(arrival_tables_df.loc[race_id, :][arrival_tables_df.loc[race_id, 'horse_number'] == race_proba.iat[j, 0]]['rank']) == 1:
                        real_arrival.append(
                            str(arrival_tables_df.loc[race_id, :][
                                    arrival_tables_df.loc[race_id, 'horse_number'] == race_proba.iat[j, 0]
                                ]['rank'].iat[0])
                            )
                    else:
                        real_arrival.append('-')
                ...
                fukusho_money = 0
                for i, _proba in enumerate(_race_proba.iterrows()):
                    if '☆' in str(_proba[1]['horse_number']) and real_arrival[i].isnumeric() and int(real_arrival[i]) <= 3:
    ...
    print(better_fukusho_df.set_index(['race_id', 'horse_number']))
